...
function edit_task(task_id){
    ...
    let save_tag ="<button id='save_" + task_id + "'>Save</bbutton>";
...
function update_task(task_id){
    ...
    $.ajax({
        ...
        async: false,
        data: JSON.stringify({
            "description": value_description, "status": value_status
        })
    });
...
function add_task(){
    ...
    let view_status = $("#status_new").val();
    ...
    $.ajax({
        ...
        contentType: 'application/json;charset=UTF-8',
        async: false,
        data: JSON.stringify({"description": value_description, "status": view_status})
    });
